...
def test_create_dispatch_accept_and_done_after_registration(db_session: Session) -> None:
    service = VolunteerManagementService(db_session)
    ...
    accepted_result = service.process_message(source_context("accept"))
    done_result = service.process_message(source_context("done"))

    assert dispatch_result.dispatch_id == accepted_result.dispatch_id
    assert dispatch_result.dispatch_id == done_result.dispatch_id
    assert accepted_result.volunteer_status == VOLUNTEER_STATUS_BUSY
...
def test_dispatch_creation_marks_volunteer_pending_response(db_session: Session) -> None:
    service = VolunteerManagementService(db_session)
    registration = complete_registration(service)
    assert registration.volunteer_id is not None
    ...
    accepted = service.process_message(source_context("accept"))
    assert accepted.volunteer_status == VOLUNTEER_STATUS_BUSY
